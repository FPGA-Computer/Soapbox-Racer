...
static void configure_uart(UART_TypeDef *uart, const SerialConfig *config)
{
  ...
  // Turn on FIFO
  uart->PFIFO = UARTx_PFIFO_RXFE | UARTx_PFIFO_TXFE;
  // Flush FIFO
  uart->CFIFO = UARTx_CFIFO_TXFLUSH | UARTx_CFIFO_RXFLUSH;
...
#if KINETIS_SERIAL_USE_UART0 || defined(__DOXYGEN__)
CH_IRQ_HANDLER(UART0_RX_TX_IRQHandler) {
...
#if KINETIS_SERIAL_USE_UART1 || defined(__DOXYGEN__)
CH_IRQ_HANDLER(UART1_RX_TX_IRQHandler) {
...
#if KINETIS_SERIAL_USE_UART2 || defined(__DOXYGEN__)
CH_IRQ_HANDLER(UART2_RX_TX_IRQHandler) {
